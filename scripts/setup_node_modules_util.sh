...
export NODE_PATH=$TOOLS_DIR/node-10.16.0
...
if [ ! -d "$NODE_PATH" ]; then
  ...
  if [ ${OS} == "Darwin" ]; then
    if [ ${MACHINE_TYPE} == 'x86_64' ]; then
      NODE_FILE_NAME=node-v10.16.0-darwin-x64
    else
      NODE_FILE_NAME=node-v10.16.0-darwin-x86
    fi
  elif [ ${OS} == "Linux" ]; then
    if [ ${MACHINE_TYPE} == 'x86_64' ]; then
      NODE_FILE_NAME=node-v10.16.0-linux-x64
    else
      NODE_FILE_NAME=node-v10.16.0-linux-x86
    fi
  fi

  curl -o node-download.tgz https://nodejs.org/dist/v10.16.0/$NODE_FILE_NAME.tar.gz
